...
function Details({ order }) {
  ...
  const distance = getDistance(order.userLoc.geopoint, order.sellerLoc.geopoint);
  const price = getDistance(order.userLoc.geopoint, order.sellerLoc.geopoint, true) * 2.25;
    ...
    <View style={{ flex: 1, justifyContent: 'space-between' }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        <Button
          ...
          text={'Refuser'}
          style={{ flex: 1, marginRight: responsiveWidth(4), backgroundColor: '#EA190C', borderColor: 'transparent' }}
        />
        <Button
          onPress={onAcceptCourse}
          text={'Accepter'}
          style={{ flex: 1, backgroundColor: '#53B047', borderColor: 'transparent' }}
        />
      </View>
    </View>
  );
