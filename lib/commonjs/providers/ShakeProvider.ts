...
import React, { useGlobal, useState, useEffect, useCallback } from 'reactn';
...
Input from '../components/Inputs/Input';
...
useEffect(() => {
    const subscription = lib.addListener(data => {
...
const Header = useCallback(() => {
    return (
      <View style={[SharedStyles.containerRowSpaceBetween, {
        ...
        <Pressable onPress={() => {
          ...
          <Image resizeMode="contain" source={icons.back} style={{
            height: 30,
            width: 30,
            tintColor: Palette.mainWhite
          }} />
        </Pressable>
        <Image resizeMode="contain" source={require('../assets/images/logoFull.png')} style={{
          height: 30,
          width: 150
        }} />
        <View style={{
    ...
    <React.Fragment>
      ...
      <Modal animationType="slide" visible={showModal} onRequestClose={() => {
        ...
        <View style={{
          ...
          <Text style={[Fonts.primary.bold(17, Palette.mainWhite), {
            textAlign: 'center',
            marginBottom: 20
          }]}>
            {stepList[step].title}
          ...
          {!step ? (
            ...
            }} value={description} onChange={setDescription} isTextarea={true} />
          )}
          <Button text={step === stepList.length - 1 ? 'Envoyer' : 'Continuer'} primary={true} isAbsoluteBottom={true} textColor={Palette.mainWhite} onPress={() => {
