...
function checkIfEmailIsValid(mail) {
  const reg = /^\w+([.-]?\w+)*@\w+([.-]?\w+)*(\.\w{2,3})+$/;
  return reg.test(mail);
}

function checkIfPasswordIsStrongEnough(password) {
  const reg = /^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,})/;
  return reg.test(password);
}

const formatPhoneNumber = function () {
  let phoneNumber = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
