...
export default function MinuitCamera({
  ...
  const [camRef, setCameRef] = useState<Camera | null>(null);
  const [recording, setRecording] = useState(false);
  const [isPictureMode, setIsPictureMode] = useState(true);
  const [cameraType, setCameraType] = useState(CameraType.back);
  const [viewDimension, setViewDimension] = useState<ViewDimension | null>(null);
...
type MediaTypeSelectorProps = {
  ...
  setIsPictureMode: (isPictureMode: boolean) => void;
  disabled?: boolean;
  theme: Theme;
  style?: object;
};
